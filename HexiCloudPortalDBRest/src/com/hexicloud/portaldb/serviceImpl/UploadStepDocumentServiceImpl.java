...
public class UploadStepDocumentServiceImpl implements UploadStepDocumentService {
    ...
    @Override
    public DocumentUploadReponse uploadDocument(MultipartFile multipartFile, String parentFolderId, String fileId) throws IOException {
        ...
        if (multipartFile != null) {
            ...
            try {
                ...
                if (multipartFile != null) {
                    ...
                    FormDataBodyPart fdpjson = new FormDataBodyPart(UploadStepDocumentService.JSON_INPUT_PARAMETERS, jsonPayload);
                    multipart.bodyPart(fdpjson); //parentID

                    FileDataBodyPart fdp = new FileDataBodyPart(UploadStepDocumentService.FILE_KEY, uploadFile, MediaType.TEXT_PLAIN_TYPE);
        ...
        if (stepFolderService != null && uploadStepDocument != null && uploadStepDocument.getStepId() != null) {
            ...
            if (listOfStepFolders != null && !(listOfStepFolders.isEmpty())) {
                for (StepFolder stepFolder : listOfStepFolders) {
                    if (uploadStepDocument.getStepId() != null && (uploadStepDocument.getSubStepCode() == null || uploadStepDocument.getSubStepCode().equals(""))) {
                        if (stepFolder.getStepId() != null && stepFolder.getSubStepCode() == null) {
                            stepLevelFolderId = stepFolder.getFolderId();
                            break;
                        }
                    } else if (uploadStepDocument.getStepId() != null && (uploadStepDocument.getSubStepCode() != null && !(uploadStepDocument.getSubStepCode().equals("")))) {
                        if (stepLevelFolderId == null && stepFolder.getStepId() != null && stepFolder.getSubStepCode() == null) {
                            stepLevelFolderId = stepFolder.getFolderId();
                        }
                        if (stepFolder.getStepId() != null && stepFolder.getSubStepCode() != null) {
            ...
            if(uploadStepDocument.getStepId() != null)
            {
                ...
                if((uploadStepDocument.getSubStepCode() == null || uploadStepDocument.getSubStepCode().equals("")))
                {
                    if(stepLevelFolderId == null)
                    {
                        ...
                        {
                            CreateFolderResponse createFolderResponse = dcsFolderService.createFolder("self", this.prepareCreateStepFolderRequest(uploadStepDocument));
...
//                        }
                ...
                else if((uploadStepDocument.getSubStepCode() != null && !(uploadStepDocument.getSubStepCode().equals(""))))
                {
                    if (stepLevelFolderId == null) {
                        if(dcsFolderService != null) {
                            CreateFolderResponse createFolderResponse = dcsFolderService.createFolder("self", this.prepareCreateStepFolderRequest(uploadStepDocument));
...
//                        }
                    ...
                    if (subStepLevelFolderId == null) {
                        CreateFolderResponse createFolderResponse =
                                dcsFolderService.createFolder(stepLevelFolderId, this.prepareCreateSubStepFolderRequest(uploadStepDocument));
                        if(createFolderResponse != null)
                        {
                            folderId = createFolderResponse.getId();
