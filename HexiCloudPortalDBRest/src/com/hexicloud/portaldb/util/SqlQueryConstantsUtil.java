...
public class SqlQueryConstantsUtil {
    ...
    // All users Other Use Cases
    public static final String SQL_GET_ALL_OTHER_USECASES_WITH_JOINS =
        "SELECT UC.USECASE_ID, UC.TITLE, UC.SHORT_DESC, UC.LONG_DESC, UC.IMAGE, SER.SERVICE_ID, SER.LABEL FROM USE_CASES UC, SERVICES SER, USE_CASE_SERVICES UCS \n" + "WHERE STATUS <> 'INACTIVE' AND UC.USECASE_ID = UCS.USECASE_ID(+) AND UCS.SERVICE_ID = SER.SERVICE_ID(+) AND UC.USECASE_ID = 10";
    ...
    public static final String SQL_UPDATE_PHASE_COMPLETION_DATE =
        "UPDATE USER_PHASE_COMPLETETION SET COMPLETED_DATE = SYSDATE WHERE USER_ID = ? AND PHASE = ?";
    ...
    public static final String SQL_GET_USE_CASE_BENEFITS =
        "SELECT B.BENEFIT_ID, B.TITLE, B.SHORT_DESC, B.LONG_DESC, B.IMAGE, B.IMAGE_LINK FROM BENEFITS B, USE_CASE_BENEFITS UCB WHERE B.BENEFIT_ID = UCB.BENEFIT_ID AND UCB.USECASE_ID =?";
    ...
    // Guided Paths Queries
    public static final String SQL_GET_CORE_GUIDED_PATHS =
        //        "SELECT GP.PATH_ID, GP.LABEL, GP.SHORT_DESC, GP.IMAGE, GP.IMAGE_LINK, GP.ISCORE, SS.SERVICE_ID, SS.LABEL S_LABEL FROM GUIDED_PATHS GP, GUIDED_PATH_MAPPING GPM,  SERVICES SS WHERE GPM.SERVICE_ID IS NOT NULL AND GP.PATH_ID = GPM.PATH_ID AND GPM.SERVICE_ID = SS.SERVICE_ID AND GP.ISCORE = 'Y'";
        "SELECT GP.PATH_ID, GP.LABEL, GP.SHORT_DESC, GP.IMAGE, GP.IMAGE_LINK, GP.ISCORE, SS.SERVICE_ID, SS.LABEL S_LABEL, GPS.SECTION_ID, DOCS.SECTION_DOC_ID, DOCS.PUBLIC_LINK, DOCS.DOC_NAME, DOCS.DOC_TYPE, DOCS.DOC_DESCRIPTION, ULH.STATUS,   ULH.LAST_UPDATED_DATE, ULH.PAGE_NUMBER FROM GUIDED_PATHS GP, GUIDED_PATH_MAPPING GPM, GUIDED_PATH_SECTIONS GPS, GUIDED_PATH_SECTION_DOCS DOCS, SERVICES SS,   USER_LEARNING_HISTORY ULH WHERE GPM.SERVICE_ID IS NOT NULL AND GP.PATH_ID = GPM.PATH_ID AND GPM.SERVICE_ID = SS.SERVICE_ID AND GP.ISCORE = 'Y' AND GP.PATH_ID = GPS.PATH_ID AND GPS.SECTION_ID = DOCS.SECTION_ID AND ULH.SECTION_DOC_ID(+) = DOCS.SECTION_DOC_ID AND ULH.USER_ID(+) = ?";
    
    public static final String SQL_GET_COMPLE_GUIDED_PATHS = 
        "SELECT GP.PATH_ID, GP.LABEL, GP.SHORT_DESC, GP.IMAGE, GP.IMAGE_LINK, GP.ISCORE, GPS.SECTION_ID, DOCS.SECTION_DOC_ID, DOCS.PUBLIC_LINK, DOCS.DOC_NAME, DOCS.DOC_TYPE,DOCS.DOC_DESCRIPTION, ULH.STATUS, ULH.LAST_UPDATED_DATE, ULH.PAGE_NUMBER, GPM.USECASE_ID FROM GUIDED_PATHS GP, GUIDED_PATH_MAPPING GPM, GUIDED_PATH_SECTIONS GPS, GUIDED_PATH_SECTION_DOCS DOCS, USER_LEARNING_HISTORY ULH WHERE  GPM.USECASE_ID = ? AND GP.PATH_ID = GPM.PATH_ID AND GP.PATH_ID = GPS.PATH_ID AND GPS.SECTION_ID = DOCS.SECTION_ID AND ULH.SECTION_DOC_ID(+) = DOCS.SECTION_DOC_ID AND ULH.USER_ID(+) = ?";
    
    public static final String SQL_GET_GUIDED_PATH_DETAIL = 
        "SELECT GP.PATH_ID, GP.LABEL, GP.SHORT_DESC, GP.IMAGE, GP.IMAGE_LINK, GPS.SECTION_ID, GPS.SECTION_TITLE, GPS.DESCRIPTION, GPS.SECTION_ORDER, DOCS.SECTION_DOC_ID,   DOCS.PUBLIC_LINK, DOCS.DOC_NAME, DOCS.DOC_TYPE, DOCS.DOC_DESCRIPTION, DOCS.DOC_ORDER, DOCS.TIME_TO_COMPLETE, ULH.STATUS, ULH.LAST_UPDATED_DATE, ULH.PAGE_NUMBER FROM GUIDED_PATHS GP,   GUIDED_PATH_MAPPING GPM, GUIDED_PATH_SECTIONS GPS, GUIDED_PATH_SECTION_DOCS DOCS, USER_LEARNING_HISTORY ULH WHERE GP.PATH_ID = ? AND GP.PATH_ID = GPM.PATH_ID AND GP.PATH_ID = GPS.PATH_ID AND GPS.SECTION_ID = DOCS.SECTION_ID AND ULH.SECTION_DOC_ID(+) = DOCS.SECTION_DOC_ID AND ULH.USER_ID(+) = ?";
    
    public static final String SQL_MERGE_LEARNING_HISTORY = 
        "MERGE INTO USER_LEARNING_HISTORY ULH\n" + 
        "USING (SELECT 1 FROM DUAL) m\n" + 
        "    ON (ULH.USER_ID = :userId AND ULH.PATH_ID = :pathId AND ULH.SECTION_ID= :sectionId AND ULH.SECTION_DOC_ID = :sectionDocId)\n" + 
        "WHEN MATCHED THEN\n" + 
        "  UPDATE SET ULH.STATUS = :status, LAST_UPDATED_DATE = SYSDATE, PAGE_NUMBER = :pageNumber\n" + 
        "WHEN NOT MATCHED THEN\n" + 
        "  INSERT VALUES (:userId, :pathId , :sectionId, :sectionDocId, :status, SYSDATE, :pageNumber)";
//    public static final String SQL_GET_GUIDED_PATH_PROGRESS =
//        "SELECT GP.PATH_ID, GPS.SECTION_ID, DOCS.SECTION_DOC_ID, ULH.STATUS, ULH.LAST_UPDATED_DATE, ULH.PAGE_NUMBER FROM GUIDED_PATHS GP, GUIDED_PATH_MAPPING GPM,   GUIDED_PATH_SECTIONS GPS, GUIDED_PATH_SECTION_DOCS DOCS, SERVICES SS, USER_LEARNING_HISTORY ULH WHERE GPM.SERVICE_ID IS NOT NULL AND GP.PATH_ID = GPM.PATH_ID AND GPM.SERVICE_ID = SS.SERVICE_ID AND GP.ISCORE = 'Y' AND GP.PATH_ID = GPS.PATH_ID AND GPS.SECTION_ID = DOCS.SECTION_ID AND ULH.SECTION_DOC_ID(+) = DOCS.SECTION_DOC_ID\n" +
//        "AND ULH.USER_ID(+) = ?";
    
    
    public static final String SQL_GET_ALL_OTHER_USECASES =
        "SELECT USER_ID, SUMMARY, SERVICES, BENEFITS FROM USER_USECASES WHERE USECASE_ID = 10 AND CODE = 'I'";
}
